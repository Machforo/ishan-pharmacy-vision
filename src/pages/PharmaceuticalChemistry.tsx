...
export default function PharmaceuticalChemistryPage() {
  ...
  const pageData = data?.length > 0 ? data.find((d: any) => d.slug === "/pharmaceutical-chemistry") : null;
  ...
  const defaultSections: Record<string, React.ReactNode> = {
    header: (
      <PageHeader
        ...
        breadcrumbs={[{ label: "Labs" }, { label: "Pharmaceutical Chemistry" }]}
      />
    ),
    overview: (
    ...
    equipment_specs: (
      <section key="equipment_specs" className="py-8">
        <div className="container-wide">
  ...
  const defaultOrder = ["header", "overview", "equipment_specs", "gallery", "cta"];

  return (
    <Layout>
      <DynamicPageSections
        pageId="pharmaceutical_chemistry"
