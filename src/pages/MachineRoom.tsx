...
export default function MachineRoomLabPage() {
  const ref = useScrollReveal();
  const { data } = usePharmacyData("facilities");
  
  const fallback = {
    title: "Industrial Machine Room",
  ...
  const pageData = data?.length > 0 ? data.find((d: any) => d.slug === "/machine-room") : null;
  ...
  const defaultSections: Record<string, React.ReactNode> = {
    header: (
      <PageHeader
        ...
        breadcrumbs={[{ label: "Labs" }, { label: "Industrial Machine Room" }]}
      />
    ),
    overview: (
    ...
    industrial_machinery: (
      <section key="industrial_machinery" className="py-8">
        <div className="container-wide">
  ...
  const defaultOrder = ["header", "overview", "industrial_machinery", "gallery", "cta"];
