...
export default function PharmacyPracticeLabPage() {
  ...
  const pageData = data?.length > 0 ? data.find((d: any) => d.slug === "/pharmacy-practice") : null;
  ...
  const defaultSections: Record<string, React.ReactNode> = {
    header: (
      <PageHeader
        ...
        breadcrumbs={[{ label: "Labs" }, { label: "Pharmacy Practice Lab" }]}
      />
    ),
    overview: (
    ...
    clinical_setup: (
      <section key="clinical_setup" className="py-8">
        <div className="container-wide">
  ...
  const defaultOrder = ["header", "overview", "clinical_setup", "gallery", "cta"];

  return (
    <Layout>
      <DynamicPageSections
        pageId="pharmacy_practice"
