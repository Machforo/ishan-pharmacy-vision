...
export default function PharmacognosyLabPage() {
  ...
  const pageData = data?.length > 0 ? data.find((d: any) => d.slug === "/pharmacognosy") : null;
  ...
  const defaultSections: Record<string, React.ReactNode> = {
    header: (
      <PageHeader
        ...
        breadcrumbs={[{ label: "Labs" }, { label: "Pharmacognosy Lab" }]}
      />
    ),
    overview: (
    ...
    crude_drugs_collection: (
      <section key="crude_drugs_collection" className="py-8">
        <div className="container-wide">
  ...
  const defaultOrder = ["header", "overview", "crude_drugs_collection", "gallery", "cta"];

  return (
    <Layout>
      <DynamicPageSections
        pageId="pharmacognosy"
