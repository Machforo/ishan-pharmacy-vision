...
export default function PharmacologyLabPage() {
  ...
  const pageData = data?.length > 0 ? data.find((d: any) => d.slug === "/pharmacology") : null;
  ...
  const defaultSections: Record<string, React.ReactNode> = {
    header: (
      <PageHeader
        ...
        breadcrumbs={[{ label: "Labs" }, { label: "Pharmacology Lab" }]}
      />
    ),
    overview: (
    ...
    screening_units: (
      <section key="screening_units" className="py-8">
        <div className="container-wide">
  ...
  const defaultOrder = ["header", "overview", "screening_units", "gallery", "cta"];
